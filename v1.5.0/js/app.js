(function () {
  ...
  function customerTypeLabel(c) {
    ...
    if (DemoData.isManagedNoOrderCustomer(c, DemoData.demoSalesUser)) return '新客户';
    return '客户';
  }
  ...
  function customerTypeBadgeClass(c) {
    ...
    if (DemoData.isManagedNoOrderCustomer(c, DemoData.demoSalesUser)) return 'sc-badge--new';
  ...
  function buildCustomerPromptHtml(skillId) {
    ...
    html +=
      '<div class="sc-card__actions-inline sc-card__actions-inline--prompt">' +
      '<button type="button" class="sc-btn sc-btn--primary" data-action="open-customer-sheet">选择客户</button>' +
      '<button type="button" class="sc-btn sc-btn--ghost" data-action="customer-create-open">新建客户</button>' +
      '</div></div>';
    return html;
  }
  ...
  function executeSkillAction(skillId, opts) {
    ...
    if (skillId === 'customer-create') {
      if (!opts.skipUserMsg) pushUserMsg('新增客户');
      setTimeout(() => {
        if (window.V155 && V155.openCreateCustomerCard) {
          V155.openCreateCustomerCard('', { skipUserMsg: true });
        }
      }, opts.delayMs != null ? opts.delayMs : 300);
      return;
    }
  ...
  function renderCustomerPickCardHtml() {
    ...
    if (!items.length && q) {
      listBlock =
        '<div class="sc-customer-empty-create">' +
        '<p class="sc-customer-empty-create__title">未找到匹配客户</p>' +
        '<p class="sc-customer-empty-create__sub">「' +
        escapeHtml(q) +
        '」不在您的客户范围内</p></div>';
    } else {
      listBlock =
    ...
    return (
      ...
      listBlock +
      '<div class="sc-card__actions-inline sc-card__actions-inline--split sc-card__actions-inline--picker">' +
      '<button type="button" class="sc-btn sc-btn--ghost sc-btn--block" data-action="customer-create-open" data-spec-id="btn-customer-create">+ 新增客户</button>' +
      '</div></div>'
    );
  }
  ...
  function patchRecentSectionInDom() {
    if (window.V155 && V155.patchRecentSectionInDom) {
      V155.patchRecentSectionInDom();
      persistChatHistory();
    }
  }
  ...
  function requestFollowUpListByClick() {
    switchActiveSkill('followup', { skipSkillAnnounce: true });
    pushUserMsg('今日待跟进');
    recordRecentVisit('followup', { checkpointLabel: '待跟进列表' });
  ...
  function applyRecentVisit(skillId, customerId, checkpoint) {
    if (window.V155 && V155.applyRecentVisit) {
      V155.applyRecentVisit(skillId, customerId, checkpoint);
      return;
    }
  }

  function recordRecentVisit(skillId, opts) {
    if (window.V155 && V155.recordRecentVisit) V155.recordRecentVisit(skillId, opts);
  }

  function onMessagesClick(e) {
    const recentBtn = e.target.closest('[data-recent-skill]');
    if (recentBtn) {
      e.preventDefault();
      applyRecentVisit(
        recentBtn.getAttribute('data-recent-skill'),
        recentBtn.getAttribute('data-recent-cid') || null,
        recentBtn.getAttribute('data-recent-checkpoint') || ''
      );
      return;
    }
    onMessageClick(e);
  ...
  function onMessageClick(e) {
    ...
    if (action === 'customer-create-open') {
      const prefill = btn.getAttribute('data-prefill-name') || '';
      pushUserMsg('新增客户');
      if (window.V155 && V155.openCreateCustomerCard) {
        V155.openCreateCustomerCard(prefill, { skipUserMsg: true });
      }
      scrollMessages();
      persistChatHistory();
      return;
    }
    if (window.V155 && V155.handleCreateAction && V155.handleCreateAction(action, btn)) {
      scrollMessages();
      return;
    }
  ...
  function handleIntent(text, opts) {
    ...
    if (window.V155 && V155.isCreateCustomerIntent && V155.isCreateCustomerIntent(t)) {
      setTimeout(function () {
        V155.openCreateCustomerCard('', { skipUserMsg: true });
        scrollMessages();
        persistChatHistory();
  ...
  function renderSkills() {
    ...
    DemoData.skills.forEach((sk) => {
      ...
      if (sk.id === 'customer-create') {
        b.setAttribute('data-spec-id', 'btn-customer-create-skill');
      }
  ...
  function buildRecentSectionHtml() {
    if (window.V155 && V155.buildRecentSectionHtml) return V155.buildRecentSectionHtml();
    return '<div class="sc-welcome-section" data-spec-id="chat-recent"></div>';
  }
  ...
  function resetPageToInitial() {
    ...
    localStorage.setItem(TOKEN_KEY, '1');
    try {
      localStorage.removeItem('sc-customers-local');
      Object.keys(localStorage).forEach(function (k) {
        if (k.indexOf('sc-recent-visits:') === 0) localStorage.removeItem(k);
      });
    } catch (_) {}
    if (window.DemoData && DemoData.customers) {
      DemoData.customers = DemoData.customers.filter(function (c) {
        return !c._localCreated;
      });
    }
    location.hash = '#chat';
    route();
  ...
  function initChat() {
    ...
    document.addEventListener('change', (e) => {
      ...
      const createSelect = e.target.closest(
        '[data-spec-id="card-customer-create"] select[data-field]'
      );
      if (createSelect && window.V155 && V155.handleCreateSelectChange) {
        V155.handleCreateSelectChange(createSelect);
        persistChatHistory();
      }
    });
  ...
  function init() {
    ...
    if (window.V155 && V155.syncLocalCustomersIntoDemo) V155.syncLocalCustomersIntoDemo();
    initLogin();
    initLogoutConfirm();
    initChat();
