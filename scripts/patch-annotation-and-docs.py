...
NEW_MIDDLE = r"""
  ...
  'card-order-copy': {
    ...
    content: [
      ...
      '【明细】手风琴：收起序号/品名/小计；展开改规格/数量/单价等',
      '【合计】随编辑实时更新',
      '【操作】重选订单、确认复制 → 下单确认'
    ],
  ...
  'card-order-progress-detail': {
    ...
    content: [
      '【标题】订单号 + 状态徽章',
      '【订单头】客户、单据日期、工单生产状态、发货日期、业务员、备注、状态说明',
      '【PC 宽表明细】序号、存货编码/名称/规格、销售单位、单价、税后单价、税额、未税/税后金额、下单数量、累计退货、实际发货日期、生产单号、行备注',
      '【时间轴】未审核 → 销售审核 → 已审核 → 已完成；异常标红',
      '【操作】重选订单 → 进度选单卡'
    ],
    query: ['订单主档 + 明细行；进度节点来自订单状态种子'],
    interaction: [
      '入口：技能订单进度 / 话术查进度',
      '只读详情，无本卡内提交'
    ]
