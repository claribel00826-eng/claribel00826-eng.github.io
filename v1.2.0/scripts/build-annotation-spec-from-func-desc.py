...
SPECS: list[tuple[str, str, str, list[str]]] = [
    ...
    ("card-quote-select", "1.3.2、1.4.3", "历史报价单列表 / 选择报价单卡", ["1.3.2", "1.4.3"]),
    ...
    ("card-order-source", "1.4.2", "下单来源卡", ["1.4.2"]),
    ("card-order-pick", "1.4.4", "订单选品卡", ["1.2.3.2", "1.4.4"]),
    ("card-order-cart", "1.4.5", "订单购物车卡", ["1.4.5"]),
    ("sheet-order", "1.4.6", "下单确认卡", ["1.4.6"]),
    ("card-order-success", "1.4.7", "订单成功卡", ["1.4.7"]),
]
...
def extract_section(body: str, key: str) -> str:
    ...
    patterns = {
        ...
        "1.4.2": r"### 1\.4\.2 下单来源卡\n",
        "1.4.3": r"### 1\.4\.3 选择报价单卡[^\n]*\n",
        "1.4.4": r"### 1\.4\.4 订单选品卡[^\n]*\n",
        "1.4.5": r"### 1\.4\.5 订单购物车卡[^\n]*\n",
        "1.4.6": r"### 1\.4\.6 下单确认卡\n",
        "1.4.7": r"### 1\.4\.7 订单成功卡\n",
    }
    pat = patterns[key]
