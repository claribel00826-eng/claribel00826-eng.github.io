...
window.AnnotationSpecData = {
  ...
  'card-plan-pick': {
    ...
    content: [
      ...
      "**2. 文本归一化**",
      "比对前对需求文本与各字段统一处理：转小写 → 去空白 → 去掉常见分隔符（如 `（）()·-_，。、/` 等），便于模糊匹配。",
      "**3. 单字段模糊得分（`searchTextScore`）**",
      "对「查询词 Q」与「目标文本 T」：",
      "| 情形 | 得分 |",
      "|------|------|",
      "| Q 是 T 的连续子串 | `2000 − 起始位置 − (T长度 − Q长度) × 0.2` |",
      "| Q 的字符按顺序出现在 T 中（子序列匹配，类似 IDE 搜索） | `800 − 字符间隔数 × 4 − (T长度 − Q长度)` |",
      "| 无匹配 | `−1`（视为未命中） |",
      "**4. 需求 ↔ 产品 最终匹配分（`planMatchScoreForProduct`）**",
      "按**优先级从高到低**依次判定，命中即返回（上限 1.0）：",
      "| 优先级 | 判定方式 | 匹配分 |",
      "|--------|----------|--------|",
      "| ① 整句匹配 | 需求整句对产品全部字段拼接串做 `searchTextScore`；得分 ≥ 0 时 | `min(1, 0.72 + 整句得分 / 2500)` |",
      "| ② 品名提及 | 需求句中包含产品名称（或名称中 ≥2 字的连续片段） | **0.88** |",
      "| ③ 关键词拆分 | 见下表 | **0.45 + 0.55 × (命中词数 / 总词数)**，上限 1.0 |",
      "| ④ 整句单词 | 无法拆词时，整句作为单个词对各字段做命中判定 | 命中 **0.7**，未命中 **0** |",
      "**关键词拆分（③）细则**：",
      ...
      "3. **单词命中**：某关键词 Q 对任一产品字段 T 满足以下**任一**条件即算命中该词：",
      "   - 归一化后 T 包含 Q；或",
      "   - `searchTextScore(Q, T) ≥ 80`",
      "4. **汇总**：`匹配分 = 0.45 + 0.55 × (命中词数 ÷ 拆词总数)`",
      "**5. 入库与展示**",
      "| 环节 | 规则 |",
      "|------|------|",
      ...
      "| 业务可读口径 | 可表述为「按 xx 匹配」，xx 为实际命中的字段；字段优先级：**名称 > 描述 > 规格 > 自由项 > 自定义项**（多字段同时命中时取优先级最高者） |",
      "| 更多产品（新客户） | 全库去掉推荐区已展示项后，仍用同一匹配分**降序**排列（含弱匹配，不设下限） |",
      "| 更多产品（老客户） | 不按匹配分排序，按**品名**升序 |",
      "**6. 计算示例**",
      "| 需求文本 | 产品 | 拆词 | 命中 | 匹配分 | 展示 |",
      "|----------|------|------|------|--------|------|",
      "| 伺服电机和传动齿轮箱各2台 | 伺服电机 AC220V | 伺服电机、传动齿轮箱 | 1/2 | 0.45 + 0.55×0.5 = **0.725** | 匹配 73% |",
      "| 伺服电机和传动齿轮箱各2台 | 传动齿轮箱 ZQ350 | 同上 | 1/2 | **0.725** | 匹配 73% |",
      "| 伺服电机（整句） | 伺服电机 AC220V | — | 整句子串命中 | 接近 **0.88~1.0** | 匹配 88%+ |",
      "**每行展示**：",
      "| 字段 | 说明 |",
      "|------|------|",
      "| 品名 | 产品名称 |",
  ...
  'card-quote-source': {
    ...
    content: [
      ...
      "**内容**：",
      "- **按方案报价**：须本客户有已保存方案；无方案时按钮禁用",
      "- **直接选品报价**：不依赖方案，进入选品报价卡",
      "**字段说明**：",
      "| 字段 | 说明 |",
      "|------|------|",
      "| 方案数量提示 | 0 个：提示先完成方案速配；1 个：展示当前方案编号；多个：提示按方案报价须先选择 |",
      "**交互**：",
      "| 操作 | 结果 |",
      "|------|------|",
      "| 按方案报价 | 仅 1 个方案 → 载入方案行进入逐项报价；多个方案 → **选择方案卡** |",
  ...
  'card-order-source': {
    ...
    module: "1.4.2",
    content: [
      "**出现时机**：从技能进入且本客户**已有至少 1 份报价单**时展示；无报价单时直接进入直选。",
      "**内容**：",
      "- **按报价单**：须本客户有报价单；无则按钮禁用",
      "- **直接选品**：不依赖报价单",
      "**字段说明**：",
      "| 字段 | 说明 |",
      "|------|------|",
      "| 报价单数量提示 | 0 个：提示先完成报价；1 个：展示报价单编号与金额；多个：提示须先选择 |",
    ],
    query: [],
    interaction: [],
  },
