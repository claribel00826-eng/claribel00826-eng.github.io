...
function openChangeSheet(oid, opts) {
  ...
  function renderInventoryQtyCell(n, unit, warn) {
    const cls = warn ? ' sc-inventory-list__num--warn' : '';
    return (
      '<span class="sc-inventory-list__num' +
      cls +
      '">' +
      (Number(n) || 0).toLocaleString('zh-CN') +
      '<span class="sc-inventory-list__unit"> ' +
      App.escapeHtml(unit || '件') +
      '</span></span>'
    );
  }

  function renderInventorySummary(summary, state) {
    state = state || {};
    const zeroWarn = summary.zeroAvail > 0 ? ' sc-inventory-overview__value--warn' : '';
    const lowActive = state.lowOnly ? ' is-active' : '';
    return (
      '<div class="sc-inventory-overview">' +
      '<div class="sc-inventory-overview__item">' +
      '<span class="sc-inventory-overview__value">' +
      summary.totalProducts +
      '</span>' +
      '<span class="sc-inventory-overview__label">货品</span>' +
      '</div>' +
      '<div class="sc-inventory-overview__item">' +
      '<span class="sc-inventory-overview__value">' +
      summary.totalSku +
      '</span>' +
      '<span class="sc-inventory-overview__label">规格</span>' +
      '</div>' +
      '<button type="button" class="sc-inventory-overview__item sc-inventory-overview__item--filter' +
      lowActive +
      '" data-action="inventory-filter-low" aria-pressed="' +
      (state.lowOnly ? 'true' : 'false') +
      '">' +
      '<span class="sc-inventory-overview__value' +
      zeroWarn +
      '">' +
      summary.zeroAvail +
      '</span>' +
      '<span class="sc-inventory-overview__label">可用为 0</span>' +
      '</button>' +
      '</div>'
    );
  }
  ...
  function inventoryRowMatchesKeyword(row, keyword) {
    if (!keyword) return true;
    const code = String(row.inventoryCode || '').toLowerCase();
    const name = String(row.productName || '').toLowerCase();
    const spec = String(row.skuLabel || '').toLowerCase();
    return code.indexOf(keyword) >= 0 || name.indexOf(keyword) >= 0 || spec.indexOf(keyword) >= 0;
  }
  ...
  function filterInventoryRows(allRows, filter) {
    ...
    return allRows.filter(function (row) {
      return inventoryRowMatchesKeyword(row, keyword);
    });
  }

  function applyInventoryCardFilters(sourceRows, state) {
    state = state || {};
    let rows = sourceRows || [];
    const query = String(state.query || '').toLowerCase().trim();
    if (query) {
      rows = rows.filter(function (row) {
        return inventoryRowMatchesKeyword(row, query);
      });
    }
    if (state.lowOnly) {
      rows = rows.filter(function (row) {
  ...
  function getLastInventoryCard() {
    const cards = document.querySelectorAll('[data-spec-id="card-inventory"]');
    return cards.length ? cards[cards.length - 1] : null;
  }

  function refreshInventoryResultCard() {
    if (!inventoryCardState) return;
    const card = getLastInventoryCard();
    if (!card) return;
    const filteredRows = applyInventoryCardFilters(inventoryCardState.sourceRows, inventoryCardState);
    card.outerHTML = renderInventoryResultCard(filteredRows, inventoryCardState);
    rescanAnnotationPins();
  }

  function renderInventorySkuLine(row) {
    const low = row.available <= 0;
    const rowCls = low ? ' sc-inventory-list__sku--low' : '';
    const unit = row.salesUnit || '件';
    return (
      '<div class="sc-inventory-list__sku' +
      rowCls +
      '">' +
      '<span class="sc-inventory-list__spec">' +
      App.escapeHtml(row.skuLabel || '默认') +
      '</span>' +
      '<div class="sc-inventory-list__metric sc-inventory-list__metric--onhand">' +
      renderInventoryQtyCell(row.onHand, unit, false) +
      '</div>' +
      '<div class="sc-inventory-list__metric sc-inventory-list__metric--avail">' +
      renderInventoryQtyCell(row.available, unit, low) +
      '</div>' +
      '</div>'
    );
  }

  function renderInventoryListHead() {
    return (
      '<div class="sc-inventory-list__table-head">' +
      '<span class="sc-inventory-list__spec">规格</span>' +
      '<span class="sc-inventory-list__metric sc-inventory-list__metric--onhand">库存量</span>' +
      '<span class="sc-inventory-list__metric sc-inventory-list__metric--avail">可用量</span>' +
      '</div>'
    );
  }
  ...
  function renderInventoryFilterCard() {
    return (
      '<div class="sc-card sc-card--compact sc-card--inventory" data-spec-id="card-inventory-filter">' +
      '<div class="sc-card__head sc-card__head--compact">库存查询</div>' +
      '<p class="sc-card__meta">输入存货编码或名称后点确认；也可跳过，直接查看全部。</p>' +
      '<div class="sc-plan-query-row sc-inventory-filter__query">' +
  ...
  function renderInventoryResultCard(allRows, state) {
    state = state || {};
    ...
    const listItems = groups
      .map(function (g) {
        const skuLines = g.rows.map(renderInventorySkuLine).join('');
        return (
          '<div class="sc-inventory-list__group">' +
          '<div class="sc-inventory-list__head">' +
          '<strong class="sc-inventory-list__name">' +
          App.escapeHtml(g.productName) +
          '</strong>' +
          '</div>' +
          '<div class="sc-inventory-list__body">' +
          skuLines +
          '</div>' +
          '</div>'
        );
      ...
      .join('');
    const emptyHint = !allRows.length
      ? '<p class="sc-card__meta sc-inventory-list__empty">当前筛选条件下暂无规格记录。</p>'
      : '';
    const baseFilter = state.baseFilter || '';
    const filterLabel = baseFilter ? ' · 筛选：' + App.escapeHtml(baseFilter) : '';
    const sourceCount = (inventoryCardState && inventoryCardState.sourceRows
      ? inventoryCardState.sourceRows
      : allRows
    ).length;
    const filterMeta =
      allRows.length && (state.query || state.lowOnly)
        ? '当前显示 ' + allRows.length + ' / ' + sourceCount + ' 个规格'
        : '';
    return (
      '<div class="sc-card sc-card--compact sc-card--inventory" data-spec-id="card-inventory">' +
      '<div class="sc-card__head sc-card__head--compact">库存查询' +
      filterLabel +
      '</div>' +
      renderInventorySummary(summary, state) +
      '<div class="sc-inventory-list__toolbar">' +
      '<div class="sc-plan-query-row">' +
      '<input type="search" class="sc-input sc-input--field" id="inventory-query-input" placeholder="编码 / 名称 / 规格" value="' +
      App.escapeHtml(state.query || '') +
      '"/>' +
      '<button type="button" class="sc-btn sc-btn--ghost" data-action="inventory-query-apply">筛选</button>' +
      '</div>' +
      (filterMeta
        ? '<p class="sc-inventory-list__filter-meta">' + filterMeta + '</p>'
        : '') +
      '</div>' +
      (listItems
        ? '<div class="sc-inventory-list">' + renderInventoryListHead() + listItems + '</div>'
        : emptyHint) +
      '<div class="sc-card__actions-inline">' +
  ...
  function runInventory(opts) {
    ...
    if (!filter && !skipFilter) {
      ...
      inventoryCardState = {
        ...
        query: '',
        lowOnly: false,
        sourceRows: filterInventoryRows(rows, filter)
      };
      const filteredRows = applyInventoryCardFilters(inventoryCardState.sourceRows, inventoryCardState);
      ...
      App.pushAiHtml(
        '<p class="sc-reply-lead">为您查询 ' + filterLabel + ' 规格库存（全仓合计）：</p>' +
          renderInventoryResultCard(filteredRows, inventoryCardState)
      );
    }
    rescanAnnotationPins();
  }
  ...
  function handleAction(action, btn) {
    ...
    if (action === 'inventory-query-apply') {
      const card = btn.closest('[data-spec-id="card-inventory"]');
      const input = card ? card.querySelector('#inventory-query-input') : null;
      if (inventoryCardState) {
        inventoryCardState.query = input ? input.value.trim() : '';
        refreshInventoryResultCard();
      }
      return true;
    }
    if (action === 'inventory-filter-low') {
      if (inventoryCardState) {
