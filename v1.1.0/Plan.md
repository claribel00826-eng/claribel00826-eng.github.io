...
**标注范围**：`js/annotation-spec-scope.js`（仅文档 01 映射的 spec-id；不含方案/报价/下单、交期、分析卡等）
...
- [x] 首屏：欢迎、待跟进摘要、最近访问
- [x] 待跟进全链路 + 写跟进（含提醒日期）
- [x] 意图槽位路由（`00-意图数据填槽需求.md` · 已读到→跳转→表单字段）
- [x] 未选客户时清空记录禁用
- [ ] `?spec=1` 全模块抽验通过

## V2+（下一版本目录规划）

见 [PRD.md](./PRD.md) §3；新版本从本目录**整目录复制**为 `v1.2.0/` 后迭代。
